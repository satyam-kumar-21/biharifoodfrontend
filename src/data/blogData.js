export const blogData = [
  {
    id: '1',
    title: 'The Art of Making Perfect Thekua',
    author: 'Sunita Devi',
    date: 'May 10, 2026',
    image: '/thekua.png',
    brief: 'Discover the traditional secrets behind Bihars most famous prasad. Learn how to get that perfect crunch.',
    content: `
      <p>Thekua is more than just a sweet snack for the people of Bihar; it is a symbol of faith, tradition, and the deep-rooted culture of the region. Traditionally prepared as an offering to the Sun God during the Chhath Puja festival, this hard-crust cookie has now found fans all over the world.</p>
      
      <h3>The Ingredients</h3>
      <p>The beauty of Thekua lies in its simplicity. You only need a few high-quality ingredients:</p>
      <ul>
        <li>Whole wheat flour (Atta)</li>
        <li>Jaggery (Gud) or Sugar</li>
        <li>Ghee (Clarified Butter)</li>
        <li>Dry coconut and cardamom for flavor</li>
      </ul>

      <h3>The Traditional Method</h3>
      <p>The secret to a perfect Thekua is the 'Moyan' - the amount of ghee mixed with the flour before adding the jaggery syrup. The dough should be stiff, not soft. At Swad Bihar Ka, we still use traditional wooden molds (Sancha) to give each Thekua its iconic leaf pattern.</p>
      
      <p>Stay tuned for more recipes from the heart of Bihar!</p>
    `
  },
  {
    id: '2',
    title: 'Why Bihari Sattu is the Ultimate Superfood',
    author: 'Dr. Ramesh Kumar',
    date: 'May 05, 2026',
    image: '/nimki.png',
    brief: 'High in protein, low in glycemic index, and incredibly cooling. Find out why Sattu is gaining global recognition.',
    content: `
      <p>In the villages of Bihar, Sattu has been the "poor man's protein" for centuries. Today, nutritionists worldwide are hailing it as a superfood that rivals expensive protein powders.</p>

      <h3>What is Sattu?</h3>
      <p>Sattu is essentially roasted gram flour. The process of roasting the chickpeas before grinding them locks in the nutrients and gives it a distinct nutty aroma.</p>

      <h3>Health Benefits</h3>
      <ol>
        <li><strong>Great for Digestion:</strong> High fiber content helps in cleaning the colon.</li>
        <li><strong>Cooling Agent:</strong> A glass of Sattu Sharbat is the best way to beat the Indian summer heat.</li>
        <li><strong>Protein Powerhouse:</strong> Excellent for muscle recovery and strength.</li>
      </ol>

      <p>Whether you have it as a drink, a paratha stuffing, or the classic Litti, Sattu is a must-have in your pantry.</p>
    `
  },
  {
    id: '3',
    title: 'The Sweet Legacy of Silao Ka Khaja',
    author: 'Amit Singh',
    date: 'April 28, 2026',
    image: '/gujiya.png',
    brief: 'With over 50 crispy layers, this GI-tagged sweet from Silao has a history that dates back to the Mauryan Empire.',
    content: `
      <p>If you visit the ruins of Nalanda, you cannot leave without tasting the Silao Ka Khaja. This multi-layered, crispy sweet is a marvel of traditional confectionery.</p>

      <h3>A GI-Tagged Wonder</h3>
      <p>Silao Khaja received the Geographical Indication (GI) tag because of its unique preparation method and the local water of Silao, which is said to contribute to its crispness.</p>

      <p>Legend has it that Gautam Buddha himself was offered Khaja during his travels through the Magadh region. Each piece of Khaja has up to 52 layers, making it incredibly light and flaky.</p>
      
      <p>At Swad Bihar Ka, we bring this authentic taste of Silao directly to your home, wherever you are in India.</p>
    `
  }
];
